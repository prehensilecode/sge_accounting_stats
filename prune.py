...
colnames = ['qname', 'hostname', 'group', 'owner', 'job_name', 'job_number',
...
sgeacct_df = pd.read_csv('accounting', sep=':', names=colnames)

# keep all rows that occurred after 2023-01-01 00:00:00 = timestamp 1672549200
timestamp = time.mktime(time.strptime('2023-01-01 00:00:00',
                                      '%Y-%m-%d %H:%M:%S'))
...
wanted_cols = ['qname', 'hostname', 'owner', 'job_number', 'submission_time',
               ...
               'ru_wallclock', 'ru_utime', 'slots', 'cpu', 'mem', 'io',
               'category', 'iow', 'maxvmem']

sgeacct_df.to_csv('accounting_shorter', columns=wanted_cols, sep=':',
                  index=False)
